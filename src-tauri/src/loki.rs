...
// across APP_START, CONNECT, and GET_PLAYER in the same session - not re-derived per call.
...
const BUILT_IN_MULTI_CONFIG_VERSION: &str = "f34892307c9d4727869adf53f3afa446";
const INSTALL_ID: &str = "scraper-installid";
const BASE_URL: &str = "https://api-live.loki.snowprintstudios.com/player/player2/userId";

fn now_ms() -> String {
    SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis().to_string()
}

fn envelope(player_event_type: &str, player_event_data: Value) -> Value {
    json!({
        "playerEvent": {
        ...
        "builtInMultiConfigVersion": BUILT_IN_MULTI_CONFIG_VERSION,
...
#[tauri::command]
pub async fn fetch_player_data(user_id: String, client_secret: String, snow_id: String) -> Result<Value, String> {
    let client = reqwest::Client::new();
    let base_url = format!("{BASE_URL}/{user_id}");
    ...
    let get_player_body = envelope("GET_PLAYER", json!({ "storefrontCountryCode": "NotAvailable" }));
