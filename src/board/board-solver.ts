...
function buildModel(
  ...
  return { model, assignRefs, runVarByBoard, bonusVarByBoard, groupsByBoard, eligibleCharactersByBoardGroup };
}

function runPasses(model: LpModel): SolveResult {
  ...
  for (let i = 0; i < PASS_ORDER.length; i++) {
    ...
    result = solver.Solve(model) as SolveResult;
    if (!result.feasible) {
      throw new Error(`Board assignment solve became infeasible during the "${pass.key}" pass`);
    }
